...
class knapsack : public encore::edsl::pcfg::shared_activation_record {
public:
  ...
  encore_dc_declare(encore::edsl, knapsack, sar, par, dc, get_dc)
    ...
    return dc::stmts({
      ...
      dc::spawn2_join(
        [] (sar& s, par&, plt pt, stt st) {
          return encore_call<knapsack>(st, pt, s.e + 1, s.c, s.n - 1, s.v, &s.without);
        },
        [] (sar& s, par&, plt pt, stt st) {
          return encore_call<knapsack>(st, pt, s.e + 1, s.c - s.e->weight, s.n - 1, s.v + s.e->value, &s.with);
      }),
      dc::stmt([] (sar& s, par& p) {
...
encore_pcfg_allocate(knapsack, get_cfg)
...
void benchmark(std::string infile) {
  ...
  d.add("encore", [&] {
    encore::launch_interpreter<encorebench::knapsack>(items, capacity, n, 0, &sol);
  });
  d.add("pbbs", [&] {
