...
# To call,
#   nix-shell -p 'with (import <nixpkgs> {}); callPackage ~/Work/pbbs-sptl/script/benchmark.nix { preExistingDataFolder="/run/media/rainey/157ddd80-bedc-4915-ba50-649d191a758e/oracle-guided-data/"; }' --pure -p hwloc wget htop screen ipfs ocaml R texlive.combined.scheme-full
...
let
  ...
  cactus-stack = import ../../cactus-stack/script/default.nix { inherit pkgs; inherit fetchurl; };
  ...
  pbbs-sptl = import ../../pbbs-sptl/script/default.nix { inherit pkgs;
                                     inherit fetchurl;
                                     inherit pbench;
                                     inherit sptl;
  ...
  encore = import ./default.nix { inherit pkgs;
                                     inherit fetchurl;
                                     inherit pbench;
                                     inherit sptl;
                                     inherit pbbs-include;
...
stdenv.mkDerivation rec {
  ...
  installPhase =
    ...
    ''
      ...
      #!/bin/bash
      cp -r --no-preserve=mode ${encore}/bench/ bench/
      ${dataFolderInit}
      mkdir -p pbench/
